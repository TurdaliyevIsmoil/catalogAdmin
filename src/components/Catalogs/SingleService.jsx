...
function SingleNews() {
  ...
  const { addService, deleteService, services, updateService } =
    useDataContext();
  const { id } = useParams();
  useEffect(() => {
    setedit(() => id !== "add");
  ...
  const onSubmitHandler = (e) => {
    ...
    e.preventDefault();
    if (edit) {
      updateService({ ...readyContent, id });
    } else {
      addService(readyContent);
    }
  };
  const deleteHandler = () => {
    deleteService(+id);
  ...
  const getItem = (id) => {
    return services.filter((i) => i.id === +id)[0];
  };
    ...
    <Container onSubmit={onSubmitHandler}>
      ...
      <div className={`bundleEditor ${descLang === "uz" ? "active" : ""}`}>
        <BundledEditor
          ...
          initialValue={edit && services ? getItem(id)?.post_body : ""}
          init={{ ...EditorInit }}
        />
      </div>
      <div className={`bundleEditor ${descLang === "ru" ? "active" : ""}`}>
        <BundledEditor
          ...
          initialValue={edit && services ? getItem(id)?.post_body_ru : ""}
          init={{ ...EditorInit }}
        />
      </div>
      <br />
